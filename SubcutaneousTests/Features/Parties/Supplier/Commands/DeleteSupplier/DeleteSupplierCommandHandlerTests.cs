...
namespace SubcutaneousTests.Features.Parties.Supplier.Commands.DeleteSupplier;
...
public class DeleteSupplierCommandHandlerTests
{
    ...
    public DeleteSupplierCommandHandlerTests(WebAppFactory factory)
    {
    ...
    [Fact]
    public async Task Handle_WithExistingSupplier_ShouldSucceedAndDeleteFromDb()
    {
        var graph = await SeedCountryAndCityAsync(_context);

        var supplier = SupplierFactory.CreateValid(supplierCode: $"SUP-ff" , address:AddressFactory.CreateValid(countryId:graph.Country.Id , cityId:graph.City.Id));
        await _context.Suppliers.AddAsync(supplier);
        await _context.SaveChangesAsync(CancellationToken.None);

        var result = await _mediator.Send(new DeleteSupplierCommand(supplier.Id));

        Assert.True(result.IsSuccess);
        Assert.False(await _context.Suppliers.AnyAsync(x => x.Id == supplier.Id));
    }

    [Fact]
    public async Task Handle_WithMissingSupplier_ShouldFail()
    {
        var result = await _mediator.Send(new DeleteSupplierCommand(Guid.NewGuid()));
