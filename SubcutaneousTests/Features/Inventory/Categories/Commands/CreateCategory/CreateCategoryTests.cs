...
public class CreateCategoryTests
{
    ...
    public CreateCategoryTests(WebAppFactory factory, ITestOutputHelper output)
    {
    ...
    {
        var category = CategoryFactory.CreateValid(name: $"category");
        await _context.Categories.AddAsync(category, CancellationToken.None);
        await _context.SaveChangesAsync(CancellationToken.None);

        var command = new UpdateCategoryCommand { Id = category.Id, Name = $"New-category" };
        var result = await _mediator.Send(command, CancellationToken.None);
