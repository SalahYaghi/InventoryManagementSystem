...
public class GetCategoryPagedQueryHandlerTests
{
    private readonly IMediator _mediator;
    private readonly IAppDbContext _context;
    private readonly ITestOutputHelper _output;

    public GetCategoryPagedQueryHandlerTests(WebAppFactory factory, ITestOutputHelper output)
    {
    ...
    {
        var category = CategoryFactory.CreateValid(name: $"Old-fff");
        await _context.Categories.AddAsync(category, CancellationToken.None);
        await _context.SaveChangesAsync(CancellationToken.None);

        var command = new UpdateCategoryCommand { Id = category.Id, Name = $"New-fff" };
        var result = await _mediator.Send(command, CancellationToken.None);
