...
namespace SubcutaneousTests.Features.References.ContactInfo.Queries.GetContactInfo;

[Collection(WebAppFactoryCollection.CollectionName)]
public class GetContactInfoQueryHandlerTests
{
    private readonly IMediator _mediator;
    private readonly IAppDbContext _context;
    private readonly ITestOutputHelper _output;

    public GetContactInfoQueryHandlerTests(WebAppFactory factory, ITestOutputHelper output)
    {
    ...
    [Fact]
    public async Task Handle_WithExistingContactInfo_ShouldReturnDto()
    {
        var contactInfo = await CreateSavedContactInfoAsync();
        var result = await _mediator.Send(new global::Contract.Features.References.ContactInfos.Queries.GetContactInfo.GetContactInfoQuery(contactInfo.Id), CancellationToken.None);
        _output.WriteLine($"Result: {string.Join(", ", result.Errors.Select(e => e.Code + " " + e.Description))}");
        Assert.True(result.IsSuccess);
        Assert.Equal(contactInfo.Id, result.Value.Id);
        Assert.Equal(contactInfo.Email, result.Value.Email);
    }

    [Fact]
    public async Task Handle_WithMissingContactInfo_ShouldFail()
    {
        var result = await _mediator.Send(new global::Contract.Features.References.ContactInfos.Queries.GetContactInfo.GetContactInfoQuery(Guid.NewGuid()), CancellationToken.None);
