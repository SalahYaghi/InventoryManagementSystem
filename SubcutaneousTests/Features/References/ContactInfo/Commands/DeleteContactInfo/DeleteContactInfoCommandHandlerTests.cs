...
namespace SubcutaneousTests.Features.References.ContactInfo.Commands.DeleteContactInfo;
...
public class DeleteContactInfoCommandHandlerTests
{
    ...
    public DeleteContactInfoCommandHandlerTests(WebAppFactory factory, ITestOutputHelper output)
    {
    ...
    [Fact]
    public async Task Handle_WithExistingContactInfo_ShouldSucceedAndRemoveFromDb()
    {
        var contactInfo = await CreateSavedContactInfoAsync();
        var result = await _mediator.Send(new global::Contract.Features.References.ContactInfos.Commands.DeleteContactInfo.DeleteContactInfoCommand(contactInfo.Id), CancellationToken.None);
        _output.WriteLine($"Result: {string.Join(", ", result.Errors.Select(e => e.Code + " " + e.Description))}");
        Assert.True(result.IsSuccess);
        Assert.False(await _context.ContactInfos.AnyAsync(x => x.Id == contactInfo.Id, CancellationToken.None));
    }

    [Fact]
    public async Task Handle_WithMissingContactInfo_ShouldFail()
    {
        var result = await _mediator.Send(new global::Contract.Features.References.ContactInfos.Commands.DeleteContactInfo.DeleteContactInfoCommand(Guid.NewGuid()), CancellationToken.None);
