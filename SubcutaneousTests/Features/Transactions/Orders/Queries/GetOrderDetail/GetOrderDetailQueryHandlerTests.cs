...
namespace SubcutaneousTests.Features.Transactions.Order.Queries.GetOrderDetail;
...
public class GetOrderDetailQueryHandlerTests
{
    ...
    public GetOrderDetailQueryHandlerTests(WebAppFactory factory, ITestOutputHelper output)
    {
    ...
    [Fact]
    public async Task Handle_WithExistingOrderDetail_ShouldReturnMappedDetail()
    {
        var graph = await SeedPurchaseOrderEntityAsync(quantity: 3m);
        var detail = graph.Order.OrderDetails.First();
        var result = await _mediator.Send(new GetOrderDetailQuery(detail.Id), CancellationToken.None);
        Assert.True(result.IsSuccess, string.Join(", ", result.Errors.Select(e => e.Code)));
        Assert.Equal(detail.Id, result.Value.Id);
        Assert.Equal(detail.ProductId, result.Value.ProductId);
    ...
    {
        var result = await _mediator.Send(new GetOrderDetailQuery(Guid.NewGuid()), CancellationToken.None);
