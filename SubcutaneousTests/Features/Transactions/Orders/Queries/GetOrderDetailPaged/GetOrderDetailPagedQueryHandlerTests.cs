...
public class GetOrderDetailPagedQueryHandlerTests
{
    ...
    public GetOrderDetailPagedQueryHandlerTests(WebAppFactory factory, ITestOutputHelper output)
    {
    ...
    [Fact]
    public async Task Handle_WithExistingOrder_ShouldReturnOnlyThatOrderDetails()
    {
       ...
       _context =  scope.ServiceProvider.GetRequiredService<IAppDbContext>();
        ...
        await SeedPurchaseOrderEntityAsync(quantity: 5m);
 
        var result = await _mediator.Send(new GetOrderDetailPagedQuery(graph.Order.Id), CancellationToken.None);


        Assert.True(result.IsSuccess);
        Assert.Equal(graph.Order.Id, result.Value[0].OrderId);
    }
