...
namespace SubcutaneousTests.Features.Transactions.Order.Queries.GetOrder;
...
public class GetOrderQueryHandlerTests
{
    ...
    public GetOrderQueryHandlerTests(WebAppFactory factory, ITestOutputHelper output)
    {
    ...
    [Fact]
    public async Task Handle_WithExistingOrder_ShouldReturnMappedOrder()
    {
        var graph = await SeedPurchaseOrderEntityAsync();
        var result = await _mediator.Send(new GetOrderQuery(graph.Order.Id), CancellationToken.None);
        Assert.True(result.IsSuccess, string.Join(", ", result.Errors.Select(e => e.Code)));
        Assert.Equal(graph.Order.Id, result.Value.Id);
        Assert.Equal((Guid?)graph.Warehouse.Id, result.Value.SourceWarehouseId);
    ...
    {
        var result = await _mediator.Send(new GetOrderQuery(Guid.NewGuid()), CancellationToken.None);
