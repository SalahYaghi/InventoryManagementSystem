...
public class UpdateOrderCommandHandlerTests
{
    ...
    public UpdateOrderCommandHandlerTests(WebAppFactory factory, ITestOutputHelper output)
    {
    ...
    [Fact]
    public async Task Handle_WithValidData_ShouldSucceedAndPersistChanges()
    {
        var graph = await SeedPurchaseOrderEntityAsync();
        var newDueDate = DateTimeOffset.UtcNow.AddDays(3);
        var command = new UpdateOrderCommand { Id = graph.Order.Id, DiscountAmount = 1m, Notes = "updated notes", DueDate = newDueDate };

        var result = await _mediator.Send(command, CancellationToken.None);

        _context.ClearChangeTracker();
        Assert.True(result.IsSuccess);
        var order = await _context.Orders.FirstAsync(x => x.Id == graph.Order.Id, CancellationToken.None);
        Assert.Equal("updated notes", order.Notes);
        Assert.Equal(1m, order.DiscountAmount);
    }

    [Fact]
    public async Task Handle_WithMissingOrder_ShouldFail()
    {
        var command = new UpdateOrderCommand { Id = Guid.NewGuid(), DiscountAmount = 0m, Notes = "valid", DueDate = DateTimeOffset.UtcNow.AddDays(1) };
        var result = await _mediator.Send(command, CancellationToken.None);
    ...
    {
        var graph = await SeedPurchaseOrderEntityAsync();
        var command = new UpdateOrderCommand { Id = graph.Order.Id, DiscountAmount = -1m, Notes = "valid", DueDate = DateTimeOffset.UtcNow.AddDays(1) };
        var result = await _mediator.Send(command, CancellationToken.None);
        Assert.True(result.IsError);
    }
    ...
    {
        var graph = await SeedPurchaseOrderEntityAsync();
        var command = new UpdateOrderCommand { Id = graph.Order.Id, DiscountAmount = 0m, Notes = "valid", DueDate = DateTimeOffset.UtcNow.AddDays(-1) };
        var result = await _mediator.Send(command, CancellationToken.None);
        Assert.True(result.IsError);
    }
    ...
    public async Task Handle_WithCompletedOrder_ShouldFailBecauseOrderIsLocked()
    {
        ...
        var command = new UpdateOrderCommand { Id = graph.Order.Id, DiscountAmount = 0m, Notes = "valid", DueDate = DateTimeOffset.UtcNow.AddDays(2) };
        var result = await _mediator.Send(command, CancellationToken.None);
        Assert.True(result.IsError);
    }
}
