...
namespace SubcutaneousTests.Features.Transactions.Order.Commands.DeleteOrderDetail;

[Collection(WebAppFactoryCollection.CollectionName)]
public class DeleteOrderDetailCommandHandlerTests
{
    private readonly IMediator _mediator;
    private readonly IAppDbContext _context;
    private readonly ITestOutputHelper _output;

    public DeleteOrderDetailCommandHandlerTests(WebAppFactory factory, ITestOutputHelper output)
    {
    ...
    [Fact]
    public async Task Handle_WithExistingOrderDetail_ShouldSucceedAndDeleteDetail()
    {
        var graph = await SeedPurchaseOrderEntityAsync();
        var detailId = graph.Order.OrderDetails.First().Id;
        var result = await _mediator.Send(new DeleteOrderDetailCommand(detailId), CancellationToken.None);
        Assert.True(result.IsSuccess, string.Join(", ", result.Errors.Select(e => e.Code)));
        Assert.False(await _context.OrderDetails.AnyAsync(x => x.Id == detailId, CancellationToken.None));
    }

    [Fact]
    public async Task Handle_WithMissingOrderDetail_ShouldFail()
    {
        var result = await _mediator.Send(new DeleteOrderDetailCommand(Guid.NewGuid()), CancellationToken.None);
    ...
    public async Task Handle_WithCompletedOrder_ShouldFailBecauseOrderIsLocked()
    {
        ...
        var result = await _mediator.Send(new DeleteOrderDetailCommand(detailId), CancellationToken.None);
        Assert.True(result.IsError);
        Assert.True(await _context.OrderDetails.AnyAsync(x => x.Id == detailId, CancellationToken.None));
    }

    [Fact]
    public async Task Handle_WithCancelledOrder_ShouldFailBecauseOrderIsLocked()
    {
        var graph = await SeedPurchaseOrderEntityAsync(status: OrderStatus.Cancelled);
        var detailId = graph.Order.OrderDetails.First().Id;
        var result = await _mediator.Send(new DeleteOrderDetailCommand(detailId), CancellationToken.None);
        Assert.True(result.IsError);
        Assert.True(await _context.OrderDetails.AnyAsync(x => x.Id == detailId, CancellationToken.None));
