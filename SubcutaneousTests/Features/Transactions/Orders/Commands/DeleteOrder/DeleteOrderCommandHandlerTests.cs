...
namespace SubcutaneousTests.Features.Transactions.Order.Commands.DeleteOrder;
...
public class DeleteOrderCommandHandlerTests
{
    ...
    public DeleteOrderCommandHandlerTests(WebAppFactory factory, ITestOutputHelper output)
    {
    ...
    [Fact]
    public async Task Handle_WithExistingPendingOrder_ShouldSucceedAndDeleteOrder()
    {
        var graph = await SeedPurchaseOrderEntityAsync();
        var result = await _mediator.Send(new DeleteOrderCommand(graph.Order.Id), CancellationToken.None);
        Assert.True(result.IsSuccess, string.Join(", ", result.Errors.Select(e => e.Code)));
        Assert.False(await _context.Orders.AnyAsync(x => x.Id == graph.Order.Id, CancellationToken.None));
    }

    [Fact]
    public async Task Handle_WithMissingOrder_ShouldFail()
    {
        var result = await _mediator.Send(new DeleteOrderCommand(Guid.NewGuid()), CancellationToken.None);
    ...
    public async Task Handle_WithCompletedOrder_ShouldFailBecauseOrderIsLocked()
    {
        ...
        var result = await _mediator.Send(new DeleteOrderCommand(graph.Order.Id), CancellationToken.None);
        Assert.True(result.IsError);
        Assert.True(await _context.Orders.AnyAsync(x => x.Id == graph.Order.Id, CancellationToken.None));
    }
