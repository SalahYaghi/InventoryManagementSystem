...
namespace SubcutaneousTests.Features.Transactions.Invoice.Queries.GetInvoice;
...
public class GetInvoiceQueryHandlerTests
{
    ...
    public GetInvoiceQueryHandlerTests(WebAppFactory factory, ITestOutputHelper output)
    {
    ...
    [Fact]
    public async Task Handle_WithExistingInvoice_ShouldReturnMappedInvoice()
    {
        var graph = await SeedCompletedPurchaseOrderAsync(completed: true);
        var create = await _mediator.Send(new CreateInvoiceCommand { OrderId = graph.Order.Id }, CancellationToken.None);
        Assert.True(create.IsSuccess, string.Join(", ", create.Errors.Select(e => e.Code)));

        var result = await _mediator.Send(new GetInvoiceQuery(create.Value.InvoiceId), CancellationToken.None);

        Assert.True(result.IsSuccess);
        Assert.Equal(create.Value.InvoiceId, result.Value.InvoiceId);
    ...
    {
        var result = await _mediator.Send(new GetInvoiceQuery(Guid.NewGuid()), CancellationToken.None);
