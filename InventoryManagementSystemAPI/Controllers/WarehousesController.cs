...
namespace InventoryManagementSystemAPI.Controllers
{
    ...
    {
    [HttpGet]
    [OutputCache(Tags = [CacheEntities.Warehouse], PolicyName = nameof(AuthenticatedUserCachePolicy))]
    ...
    [OutputCache(Tags = [CacheEntities.Warehouse], PolicyName = nameof(AuthenticatedUserCachePolicy), VaryByRouteValueNames = ["warehouseId"])]
    [ProducesResponseType(typeof(Contract.Features.Inventory.Warehouses.DTOs.WarehouseDto), StatusCodes.Status200OK)]
    [ProducesResponseType(typeof(ProblemDetails), StatusCodes.Status401Unauthorized)]
    [ProducesResponseType(typeof(ProblemDetails), StatusCodes.Status403Forbidden)]
    ...
    [HttpPost]
    [ProducesResponseType(typeof(Contract.Features.Inventory.Warehouses.DTOs.WarehouseDto), StatusCodes.Status201Created)]
    [ProducesResponseType(typeof(ProblemDetails), StatusCodes.Status401Unauthorized)]
    [ProducesResponseType(typeof(ProblemDetails), StatusCodes.Status403Forbidden)]
    ...
    public async Task<IActionResult> Create([FromBody] CreateWarehouseRequest request, CancellationToken ct)
        {
            var result = await sender.Send(new CreateWarehouseCommand
            {
                ...
                Code = request.Code,
                Address = new 
                Contract.Features.References.Addresses.Commands.CreateAddress.CreateAddressCommand()
                {
    ...
    [MapToApiVersion("1.0")]
    [Authorize(Policy = PolicyNames.WarehouseUpdate  , Roles = RoleConstants.Admin + "," + RoleConstants.WarehouseUser)]
    public async Task<IActionResult> Update(Guid warehouseId, [FromBody] UpdateWarehouseRequest request, CancellationToken ct)
        {
            var result = await sender.Send(new UpdateWarehouseCommand
            {
                ...
                Code = request.Code,
                Address =  request.Address == null ? null :  new Contract.Features.References.Addresses.Commands.UpdateAddress.UpdateAddressCommand()
                {
                    BuildingNumber = request.Address.BuildingNumber,
                    CityId = request.Address.CityId,
                    CountryId = request.Address.CountryId,
