...
using Contract.Features.References.ContactInfos.Commands.DeleteContactInfo;
...
using Contract.Features.References.ContactInfos.Queries.GetContactInfo;
using Contract.Features.References.ContactInfos.Queries.GetContactInfoPaged;
using MediatR;
...
{
    [HttpGet]
    [OutputCache(Tags = [CacheEntities.ContactInfo], PolicyName = nameof(AuthenticatedUserCachePolicy), VaryByQueryKeys = ["pageNumber", "pageSize"])]
    ...
    [OutputCache(Tags = [CacheEntities.ContactInfo], PolicyName = nameof(AuthenticatedUserCachePolicy), VaryByRouteValueNames = ["contactInfoId"])]
    [ProducesResponseType(typeof(object), StatusCodes.Status200OK)]
    [ProducesResponseType(typeof(ProblemDetails), StatusCodes.Status401Unauthorized)]
    [ProducesResponseType(typeof(ProblemDetails), StatusCodes.Status403Forbidden)]
