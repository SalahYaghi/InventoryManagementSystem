...
using Contract.Features.Inventory.Product.Commands.CreateProductImage;
...
using Contract.Features.Inventory.Product.Queries.GetAllProductImages;
...
public sealed class ProductsController(ISender sender) : ApiController
{
    ...
    [HttpGet]
    [OutputCache(Tags = [CacheEntities.Product], PolicyName = nameof(AuthenticatedUserCachePolicy), VaryByQueryKeys = ["pageNumber", 
        "pageSize", "excludeSupplierId", "excludeProductsIds", "fromWarehouseId", "fromSupplierId"])]
    ...
    [OutputCache(Tags = [CacheEntities.Product], PolicyName = nameof(AuthenticatedUserCachePolicy), VaryByRouteValueNames = ["productId"])]
