...
using Contract.Features.Parties.Customers.Queries.GetCustomer;
...
{
    [HttpGet]
    [OutputCache(Tags = [CacheEntities.Customer], PolicyName = nameof(AuthenticatedUserCachePolicy))]
    ...
    {
        var result = await sender.Send(new Contract.Features.Parties.Customers.Queries.GetCustomerPaged.GetCustomerQuery {  }, ct);
        return result.Match(response => Ok(response), Problem);
    }

    [HttpGet("{customerId:guid}", Name = "GetCustomerById")]
    [OutputCache(Tags = [CacheEntities.Customer], PolicyName = nameof(AuthenticatedUserCachePolicy), VaryByRouteValueNames = ["customerId"])]
    [ProducesResponseType(typeof(object), StatusCodes.Status200OK)]
    [ProducesResponseType(typeof(ProblemDetails), StatusCodes.Status401Unauthorized)]
    [ProducesResponseType(typeof(ProblemDetails), StatusCodes.Status403Forbidden)]
    ...
    {
        var result = await sender.Send(new Contract.Features.Parties.Customers.Queries.GetCustomer.GetCustomerQuery(customerId), ct);
        return result.Match(response => Ok(response), Problem);
    }
