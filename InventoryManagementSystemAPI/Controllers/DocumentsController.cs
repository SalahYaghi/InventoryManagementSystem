...
using
    ...
    Contract.Features.Parties.Person.Queries.GetPersonImage;
 using Contract.Features.References.Documents.Commands.CreateDocument;
using Contract.Features.References.Documents.Commands.DeleteDocument;
using Contract.Features.References.Documents.Commands.UpdateDocument;
using Contract.Features.References.Documents.DTOs;
using Contract.Features.References.Documents.Queries.GetDocument;
using Contract.Features.References.Documents.Queries.GetDocumentPaged;
...
{
    [HttpGet]
    [OutputCache(Tags = [CacheEntities.Document], PolicyName = nameof(AuthenticatedUserCachePolicy), VaryByQueryKeys = ["pageNumber", "pageSize"])]
    ...
    [OutputCache(Tags = [CacheEntities.Document], PolicyName = nameof(AuthenticatedUserCachePolicy), VaryByRouteValueNames = ["documentId"])]
    [ProducesResponseType(typeof(object), StatusCodes.Status200OK)]
    [ProducesResponseType(typeof(ProblemDetails), StatusCodes.Status401Unauthorized)]
    [ProducesResponseType(typeof(ProblemDetails), StatusCodes.Status403Forbidden)]
