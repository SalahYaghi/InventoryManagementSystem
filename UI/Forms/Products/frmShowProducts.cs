...
namespace UI.Forms.Products
{
        ...
        private void ApplyCurrentView()
            ...
            dgvProducts.DgvCustom.SetColumnHeader("Quantity", "Quantity");
            dgvProducts.DgvCustom.SetColumnHeader("ReservedQuantity", "Reserved");
            dgvProducts.DgvCustom.SetColumnHeader("TotalQuantity", "Total");
            dgvProducts.DgvCustom.FormatColumnAsCurrency("SellingPrice");
            dgvProducts.DgvCustom.FormatColumnsAsQuantity("Quantity", "ReservedQuantity", "TotalQuantity", "MinimumStockLevel");

            }
        public async Task<ApiResult<PaginatedList>> LoadData(int pageNumber, int pageSize) {

            if (_warehouseId == Guid.Empty)
            {
                _allProducts = new List<WarehouseStockDtoForList>();
                dgvProducts.SetData(_allProducts);
                return "No warehouse is assigned to your account.";
            }
        ...
        private void LoadFilterSources()
        {
            ...
            try
            {
                cmbCategory.LoadData<WarehouseStockDtoForList>(_allProducts, p => p.Category);
                cmbUnit.LoadData<WarehouseStockDtoForList>(_allProducts, p => p.Unit);

                cmbOrderBy.LoadData(dgvProducts.DgvCustom.GetColumnNamesExcept(new HashSet<string>
                {
        ...
        private async void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            dgvProducts.Status = "Deleting product...";
            ...
            if (!result.IsSuccess)
            {
                dgvProducts.Status = "Failed to delete product";
