...
namespace UI.Services
{
    ...
    {
        public static async Task<ApiResult<PaginatedList<AdjustmentForListDto>>> GetAll(int pageNumber = 1, int pageSize = 60)
        {
            var response = await _inventoryClient.GetAsync($"{AdjustmentsRoute}?pageNumber={pageNumber}&pageSize={pageSize}");
            return await ReadResponse<PaginatedList<AdjustmentForListDto>>(response);
        }
