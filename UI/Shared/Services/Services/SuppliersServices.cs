...
namespace UI.Services
{
    ...
    {
        public static async Task<ApiResult<List<SupplierDto>>> GetAll(int pageNumber = 1, int pageSize = 60)
        {
            var response = await _inventoryClient.GetAsync($"{SuppliersRoute}?pageNumber={pageNumber}&pageSize={pageSize}");
            return await ReadResponse<List<SupplierDto>>(response);
        }
