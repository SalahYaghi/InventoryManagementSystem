...
namespace UI.Services
{
    ...
    {
        public static async Task<ApiResult<List<WarehouseStockDtoForList>>> GetByWarehouse(Guid warehouseId, int pageNumber = 1, int pageSize = 60)
        {
            var response = await _inventoryClient.GetAsync($"{ByWarehouse(warehouseId)}?pageNumber={pageNumber}&pageSize={pageSize}");
            return await ReadResponse<List<WarehouseStockDtoForList>>(response);
        }
