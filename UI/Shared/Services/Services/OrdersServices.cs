...
namespace UI.Services
{
    ...
    {
        public static async Task<ApiResult<List<OrderForListDto>>> GetAll(int pageNumber = 1, int pageSize = 60)
        {
            var response = await _inventoryClient.GetAsync($"{OrdersRoute}?pageNumber={pageNumber}&pageSize={pageSize}");
            return await ReadResponse<List<OrderForListDto>>(response);
        }
